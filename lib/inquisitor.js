...
exports.inquireOne = function(service){
	...
	if(inspector){
		inquiryDeferred = inspector.inspect(service);
	} else {
		inquiryDeferred = Q.defer().reject("unknown service protocol "+service.protocol+', try ' + _.keys(inspectors).join(' or '));
	}
	...
	var timeout = setTimeout(function(){
		...
		inquiryDeferred.reject(err);
	}, 0.9 * config.pingInterval);

	return inquiryDeferred.promise
		.then(function(){
